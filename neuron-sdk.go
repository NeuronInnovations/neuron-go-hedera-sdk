...
// AddNewConnection establishes a new connection to a peer using the provided address.
// This function is useful for buyers who want to connect to specific sellers.
//
// Parameters:
//   - ctx: The context for the connection operation
//   - p2pHost: The libp2p host instance
//   - buffers: The node buffers instance for tracking connections
//   - protocol: The protocol ID to use for the connection
//   - peerAddr: The multiaddr string of the peer to connect to (e.g., "/ip4/1.2.3.4/udp/1234/quic-v1/p2p/QmPeerID")
//
// Returns:
...
// Example:
//
//	err := sdk.AddNewConnection(ctx, p2pHost, buffers, protocol, "/ip4/1.2.3.4/udp/1234/quic-v1/p2p/QmPeerID")
//	if err != nil {
...
func AddNewConnection(
	...
	// Convert address string to AddrInfo
	addrInfo, err := peer.AddrInfoFromString(peerAddr)
	if err != nil {
		return fmt.Errorf("failed to parse peer address: %v", err)
	}

	// Try hole punching first if needed
	err = commonlib.HolePunchConnectIfNotConnected(ctx, p2pHost, *addrInfo, true)
	if err != nil {
		log.Printf("Hole punching failed, attempting direct connection: %v", err)
	}

	// Establish connection
	err = commonlib.InitialConnect(ctx, p2pHost, *addrInfo, buffers, protocol)
	if err != nil {
